...
app.post("/api/exercise/new-user", function(req, res) {
  ...
  UserList.find({ username: inputUsername }, function(err, found) {
    if (found.length == 1) {
      ...
      UserList.create({ username: inputUsername, userId: shortIdNew }, function(
        err,
        data
      ) {
...
app.get("/api/exercise/log", function(req, res) {
  if (shortid.isValid(req.query.userId) == false) {
    ...
    UserList.findOne({ userId: req.query.userId }).exec(function(
      err,
      matchingUserDoc
    ) {
      if (matchingUserDoc) {
    ...
    UserList.findOne({ userId: req.query.userId }, function(
      err,
      matchingUserDoc
    ) {
      if (matchingUserDoc) {
    ...
    UserList.findOne({ userId: req.query.userId }, function(
      ...
      if (matchingUserDoc) {
        ...
        if (req.query.from && req.query.to) {
          ...
          newlogArr = matchingUserDoc["log"].filter(
            logObj =>
              Date.parse(logObj.date) > Date.parse(filteredDoc.from) &&
              Date.parse(logObj.date) < Date.parse(filteredDoc.to)
          );
          ...
          newlogArr = matchingUserDoc.log.filter(
            logObj => Date.parse(logObj.date) > Date.parse(filteredDoc.from)
          );
        } else if (!req.query.from && req.query.to) {
          // console.log("path 9c")
          ...
          newlogArr = matchingUserDoc.log.filter(
            logObj => Date.parse(logObj.date) < Date.parse(filteredDoc.to)
          );
